...
découpée en $K$ symboles source, augmentés de $S$ symboles de contrôle de
précodage LDPC, et l'émetteur produit un flux illimité de symboles encodés ; le
...
fichier en $K$ blocs source, applique un précodage LDPC de type
Raptor~\cite{shokrollahi2006raptor} pour obtenir $L=K+S$ entrées précodées, et
...
optionnellement un canal TCP pair-à-pair direct), désactiver les chemins en
...
Robust Soliton, précodage LDPC et décodage par épluchage---ainsi que ses
paramètres (\S\ref{sec:codec}).
...
récepteur via TCP brut. Les deux parties partagent un identifiant de session
sur 64 bits.
...
flux ordonné. Après décodage, elle envoie \textsc{Ready} sur chaque lien puis
\emph{vidange} chaque socket jusqu'à EOF avant de fermer---fermer avec des
...
$(i{+}1)\bmod K$. Les $L=K+S$ symboles précodés sont les entrées d'un encodeur
LT~\cite{luby2002lt} qui tire son degré de la distribution Robust Soliton avec
$c=0{,}1$, $\delta=0{,}05$ et $R=c\sqrt{K}\ln(K/\delta)$.
...
amorce le processus d'épluchage avec les $S$ équations de contrôle et itère les
équations de degré un : résoudre un symbole le XOR-e hors de chaque équation
qui le mentionne, et une équation de degré un se résout immédiatement. La
reconstruction se termine lorsque les $K+S$ entrées sont résolues ; la charge
...
précodage LDPC, phase systématique) à plus petite échelle et avec une graine de
...
Les travaux en cours et futurs incluent des contrôles HDPC de type RaptorQ par-
dessus le précodage LDPC pour un surcoût quasi optimal à faible $K$ ; et un
chemin direct basé sur QUIC en alternative au TCP. Le relais est déjà
agnostique au transport, et le protocole est assez compact pour transiter sur
tout canal ordonné ou non.
...
canal de données SCTP avec le même codec fontaine compilé en WASM, avec un
échange manuel de SDP pour la signalisation. Une leçon pratique est ressortie
...
navigateur et par un test d'intégration sur canal simulé.
