...
mentions it, and a degree-one equation resolves immediately. The peeling step
is index-based---a reverse input-to-equation index lets each resolved symbol
touch only the equations that mention it ($O(\text{degree})$ per symbol)
instead of scanning all active equations, which keeps the browser WASM decoder
fast at high $K$. The dense HDPC
...
the same payload completes in $\approx 0.13$\,s.
...
recipe (LT over an LDPC precode plus a RaptorQ-style HDPC layer, systematic
phase) at smaller scale and with a
...
tests (including the automated share-link flow). The current build uses
host-only ICE candidates, so the automated flow is validated on a single host
(loopback) and over a LAN; public-internet peer-to-peer requires STUN/TURN,
which---together with a QUIC/WebTransport datagram transport as a higher
throughput alternative to the SCTP data channel, and a relay fallback when
direct connectivity fails---is the subject of ongoing work.
